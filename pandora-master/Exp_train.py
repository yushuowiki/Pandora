...
from Code_models import GCN, MLP, Pandora
#---------------------------     main process     ---------------------------#
...
flags.DEFINE_string('dataset', 'covid', 'Dataset string.')  
flags.DEFINE_string('model', 'Pandora', 'Model string.')  # 'gcn', 'gcn_cheby', 'dense', 'Pandora'

flags.DEFINE_float('learning_rate', 0.01, 'Initial learning rate.')
flags.DEFINE_integer('epochs', 300, 'Number of epochs to train.')
flags.DEFINE_float('dropout', 0.2, 'Dropout rate (1 - keep probability).') #0.2
...
flags.DEFINE_list('property_embedding_hidden', [64], 'the hidden layer number of property embedding')
flags.DEFINE_list('motif_embedding_hidden', [64], 'the hidden layer number of motif embedding')
flags.DEFINE_list('integration_hidden', [], 'the hidden layer number of integration')
flags.DEFINE_string('embeding_combination_method', "Connection", 'the method of embedding combination')
...
print(FLAGS.integration_hidden)
# Load data
...
def evaluate2(features, support, labels, mask, motiffeatures, placeholders):
    t_test = time.time()
    feed_dict_val = construct_feed_dict(features, support, labels, mask, motiffeatures, placeholders)
...
for epoch in range(FLAGS.epochs):
    ...
    if epoch > FLAGS.early_stopping and cost_val[-1] > np.mean(cost_val[-(FLAGS.early_stopping+1):-1]):   #-0.5
        print(cost_val[-1])
        print(np.mean(cost_val[-(FLAGS.early_stopping+1):-1]))
...
test_cost, test_acc, test_output,test_duration = evaluate2(features, support, y_test, test_mask, motiffeatures, placeholders)

print("Test set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "output=",test_output,"time=", "{:.5f}".format(test_duration))
